...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
nr_blit(nr, x = sample(300, 15), y = sample(200, 15), dino[[1]])
...
Included with `{nara}` are 16 frames of cartoon dinosaur animation from
the BubbleBobble video game - see `dino` data.
...
In this example, the `dino` sprite is renderd to a `nativeRaster` image.
This *in-memory* buffer is then displayed to the user using
`grid.raster()`.

By altering the position and animation frame every time the dino is
...
for (i in -30:110) {
  ...
  nr_blit(nr, i, 1, dino[[(i %% 16) + 1]]) # copy dino to nativeRaster
...
for (frame in 1:1000) {
  ...
  nr_fill(nr, 'grey80') 
  nr_blit(nr, x, y, dino[[(frame) %% 16 + 1]])
