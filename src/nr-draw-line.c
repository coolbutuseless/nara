...
// @param color color
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void nr_line(uint32_t *nr, int nr_width, int nr_height, int x1, int y1, int x2, int y2, uint32_t color, double thickness) {
  ...
  if (thickness > 1) {
    ...
    int x[2] = {x1, x2};
    int y[2] = {y1, y2};
    nr_polyline(nr, nr_width, nr_height, x, y, npoints, color, thickness, mitre_limit, close);
    return;
  }
...
SEXP nr_line_(SEXP nr_, SEXP x1_, SEXP y1_, SEXP x2_, SEXP y2_, SEXP color_, SEXP thickness_) {
  ...
  for (int i = 0; i < N; i++) {
    nr_line(nr, nr_width, nr_height, x1[i], y1[i], x2[i], y2[i], color[i], thickness[i]);
  }
